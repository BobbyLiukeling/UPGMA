...
def combined(data,index_row,index_clo):
    list_renew = (((data.loc[index_row] + data[index_clo]).fillna(value=0) + (
            data.loc[index_row] + data.loc[index_clo]).fillna(value=0) + (
                           data[index_row] + data[index_clo]).fillna(value=0)) / 2).drop([index_clo, index_row],
                                                                                         axis=0)
...
def run(shape):
    ...
    #
    #
    # print("UP_Double: ",(f-e)/(b-a))
    # print("UP_DoubleX: ",(h-g) / (b - a))

    return {'UPGMA':time1,'UPGMA_Double':time2,'UPGMA_Triple':time3}
